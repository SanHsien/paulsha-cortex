...
2. 啟動 manager/monitor 並檢查 service/runtime 狀態：
   ...
   PSC_CLAUDE_EXECUTABLE=/home/operator/.local/bin/claude-gemma4
   ```
   ...
   `executable_path` provenance。重跑 `cortex install service` 可將 process env 的
   設定寫入該 instance 的 manager env。
...
- `repo` 為 optional 顯式歸屬宣告（`owner/repo`，#469）：宣告後派工會寫進 builder/reviewer job 的 `workflow_repo`，`recent_done`／`slices` 的 repo 歸屬即投影此值；未宣告維持 `null`，不從本機路徑或 git remote 推斷。非法 shape（不是恰一個 `/` 或任一段為空）會 fail-closed 落 `hold`。以 work item 產生 deck spec 時，把該 work item 已確認的 repo 直接傳入：

  ```console
  cortex deck compile feature-oneshot --task "demo feature" --change demo \
    --repo owner/repo --allow-external --emit
  ```
