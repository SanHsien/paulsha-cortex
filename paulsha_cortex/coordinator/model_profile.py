...
def _create_profile_runs_root(base_runs_root: Path) -> Path:
    """Atomically reserve a durable profile run directory.

    Concurrent profile commands can share the same second-resolution stamp.
    ``mkdir`` is the authority for name ownership; a preceding ``exists`` check
    would leave a TOCTOU window where one command fails after both choose the
    same path.
    """
...
def run_model_profile(
    ...
    *,
    ...
    for identity, persona, persona_measurable in _profile_cells(registry, measured_personas):
        ...
        runs_root = _create_profile_runs_root(base_runs_root)
