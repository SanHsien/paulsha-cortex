...
| P1 | Immutable evidence/backup 在 Windows 先 chmod hardlink，read-only attribute 會讓暫存名稱無法 unlink。 | 統一為 link → unlink temp name → chmod retained target；失敗時恢復 writable 後精準清理。 |
...
- 最終 Windows gate：`pwsh -File tools/dev_check.ps1` 通過，結果為 `2005 passed, 72 skipped, 32 subtests passed in 336.99s`；wheel/sdist build 成功，兩個 artifacts 的 `twine check --strict` 均為 PASSED。此 bullet 是 gate 後追加的文件 evidence，不改 runtime/package input。
