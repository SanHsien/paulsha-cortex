...
def test_windows_python_shebang_uses_current_interpreter(tmp_path: Path) -> None:
    script = tmp_path / "patchmud"
    script.write_text("#!/usr/bin/env python3\n", encoding="utf-8")

    assert mp._patchmud_command(script, platform="nt") == [sys.executable, str(script)]


def test_windows_native_launcher_remains_direct(tmp_path: Path) -> None:
    launcher = tmp_path / "patchmud.exe"

    assert mp._patchmud_command(launcher, platform="nt") == [str(launcher)]


def test_windows_discovers_extensionless_python_shebang_on_path(
    fake_patchmud: SimpleNamespace, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(mp.shutil, "which", lambda *_args, **_kwargs: None)
    monkeypatch.setenv("PATH", str(fake_patchmud.tools))
    options = mp.ProfileOptions(
        patchmud_root=fake_patchmud.root,
        registry_file=fake_patchmud.registry,
    )
    ...
    assert result["patchmud"]["bin"] == str(fake_patchmud.bin)
...
def test_profile_runs_root_collision_reserves_distinct_directory(tmp_path: Path) -> None:
    base = tmp_path / "runs" / "profile-claude-sonnet-20260812T000000Z"

    first = mp._create_profile_runs_root(base)
    second = mp._create_profile_runs_root(base)

    assert first == base
    assert second == base.with_name(f"{base.name}-2")
