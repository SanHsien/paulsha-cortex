...
class TestRecoverPreCandidateOnFreshFailure:
    ...
    def test_recover_pre_candidate_cleanly_resets_failed_failed_slice(
        ...
        # `recover-pre-candidate` 會走 `worktree_reclaim`，其預設 git runner 打的是
        # `paths.repo_root()`。舊實作未宣告 `PSC_REPO_ROOT` 時退回 `Path.cwd()`
        # ＝ operator 的真實 checkout，因此這個測試以前是在**真 repo** 上跑
        # `git worktree list --porcelain`。改成自備一個空 repo 並顯式宣告。
