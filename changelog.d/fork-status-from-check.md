...
- `docs/FORK.md` 的水位快照移除寫死的 `ahead 44 / behind 202`。那個數字隨每次 commit 變動，寫下的當下就開始過期（同日再合併兩個 PR 後實際已是 56）。文件只保留穩定的 SHA 與決策，分岔量指向 workflow run summary 的 Fork status 或本機執行。
